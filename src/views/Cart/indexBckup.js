...
class Cart extends Component {
  ...
  GoCheckouttoggle() {
    ...
    this.setState((prevState) => ({
      GoCheckoutmodal: !prevState.GoCheckoutmodal,
    }));
  }
  ...
  showfailedToggle() {
    this.setState((prevState) => ({
      showfailedModal: !prevState.showfailedModal,
    }));
  }
  ...
  checkAll() {
    if (this.state.isChecked === false) {
      ...
      let newCartData = this.state.added_item.map((item) => ({
        ...item,
        isChecked: true,
      }));
      ...
      for (var i = 0; i < this.state.added_item.length; i++) {
        ...
        var amount_cal =
          this.state.added_item[i].your_price *
          this.state.added_item[i].quantity;
        subtotal_amount = subtotal_amount + amount_cal;
        var gst_cal =
          parseInt(this.state.added_item[i].gst_value) *
          this.state.added_item[i].quantity;
        subtotal_gst = subtotal_gst + gst_cal;
        if (this.state.added_item[i].weight) {
          var weight =
            Number(this.state.added_item[i].weight) *
            Number(this.state.added_item[i].quantity);
  ...
  RFQMessagetoggle() {
    this.setState((prevState) => ({
      RFQMessagemodal: !prevState.RFQMessagemodal,
    }));
  }
  ...
  getPaymentData() {
    var url = "";
    if (window.location.href === zwzurl + "cart") {
      url = zwzapiurl + "api/get_avlinfo/";
    } else if (
      window.location.href === "https://store.nodbearings.net/cart" ||
      window.location.href === "https://localhost:3000/cart" ||
      window.location.href === "https://store.nodbearings.net/cart#"
    ) {
      url = "https://api.store.nodbearings.net/api/get_avlinfo/";
    } else {
      url = zwzapiurl + "api/get_avlinfo/";
    }
    axios
  ...
  async onUpdateQuantity(e, itemId, Quantity) {
    ...
    if (
      ...
      axios
        .post(
          zwzapiurl + "api/update_item_quantity/",
          {
            item_id: itemId,
        ...
        .catch(function (error) {});
    } else if (window.location.href === "https://store.nodbearings.net/cart") {
      axios
        .post(
          "https://api.store.nodbearings.net/api/update_item_quantity/",
          {
            item_id: itemId,
            item_quantity: Quantity,
          },
  ...
  async getCartData() {
    ...
    if (window.location.href === zwzurl + "cart") {
      ...
      axios
        .get(zwzapiurl + "api/display_additem/", {
          headers: {
            Authorization: "Token " + localStorage.getItem("auth_key"),
          },
        })
        .then((response) => {
          ...
          */
          ...
          let newCartData = response.data.itemdetails.map((item) => ({
            ...item,
            isChecked: false,
          }));
...
console.log("after_login" + totalAmount1);
          ...
          if (response.data.success === true) {
            ...
            for (var i = 0; i < item_length; i++) {
              if (
                response.data.itemdetails[i].Availability == "Stock" &&
                response.data.itemdetails[i].amount_per_unit > 0
              ) {
                /*this.setState({
                showPrice: true,
                hidePrice:false
                ...
                /* var amount_cal = response.data.itemdetails[i].total_amount*/
                var amount_cal =
                  response.data.itemdetails[i].your_price *
                  this.state.added_item[i].quantity;
                subtotal_amount = subtotal_amount + amount_cal;
              } else {
                count = count + 1;
...
cart: response.data.itemdetails.length,
  ...
  async removeItem1(itemCode1) {
    //console.log(this.props.cart);
    var arrayCartData = this.props.cart.filter(
      (item) => item.itemid !== itemCode1.itemid
    );
    console.log(arrayCartData);
    this.props.dispatch(removeCartData([...arrayCartData]));
  }

  async removeItem(itemId) {
    if (
      window.location.href === zwzurl + "cart" ||
      window.location.href.indexOf("https://localhost:3000/cart") > -1
    ) {
      axios
        .post(
          zwzapiurl + "api/remove_item_cart/",
      ...
      axios
        .post(
          "https://api.store.nodbearings.net/api/remove_item_cart/",
  ...
  async goToRFQ1() {
    if (
      ...
      axios
        .post(
          zwzapiurl + "api/check_rfq_status/",
          {
            item_info: arr,
          },
          {
      ...
      axios
        .post(
          "https://api.store.nodbearings.net/api/check_rfq_status/",
    ...
    if (
      ...
      for (var i = 0; i < this.state.added_item.length; i++) {
        if (this.state.added_item[i].isChecked == true) {
          ...
          orderData["descript"] = this.state.added_item[i].descript;
          orderData["quantity"] = this.state.added_item[i].quantity;
          orderData["amount_per_unit"] =
            this.state.added_item[i].amount_per_unit;
          /*orderData['total_amount'] = this.state.added_item[i].total_amount;*/
          orderData["total_amount"] = this.state.added_item[i].total_value1;
          orderData["added_on"] = this.state.added_item[i].added_on;
          orderData["brandname"] = this.state.added_item[i].brandname;
      ...
      axios
        .post(
          zwzapiurl + "api/sales_confirmation/",
          {
            item_info: arr,
          },
          {
      ...
      axios
        .post(
          "https://api.store.nodbearings.net/api/sales_confirmation/",
          {
            item_info: arr,
          },
          {
  ...
  render() {
    const { Successmodal, showupdateModal, showFailedModal, RFQMessagemodal } =
      this.state;
    const { Failedmodal, shippingModal, GoRFImodal, GoCheckoutmodal } =
      this.state;
    return (
      <div>
        <Header {...this.props} cart_num={this.state.no_cart_data}>
          {" "}
        </Header>
        {this.state.isActive && (
          <div
        ...
        <div
          className="content-container wrapper"
          style={{ backgroundColor: "#f4f4f4", padding: 10, marginTop: 185 }}
        >
          <Row>
            {(this.state.added_item.length > 0 ||
              (this.props.cart.length > 0 && !this.state.isActive)) && (
            ...
            {this.state.added_item.length == 0 &&
              ...
              !this.state.isActive && (
                <h3 style={{ color: "#444", paddingLeft: 12 }}>
                  {" "}
                  Your Cart is empty{" "}
                  <img src={emptyCart} style={{ width: 45, height: 45 }} />{" "}
                </h3>
              )}
                              ...
                              this.state.added_item.map((item, index) => (
                                <tr className="cart-data">
                                  ...
                                  {item.your_price > 0 ? (
                                    <td>
                                      ...
                                      <span style={{ color: "#00739E" }}>
                                        {" "}
                                        Savings:{" "}
                                        {item.calculate_discount_percentage}%
                                      </span>{" "}
                                      <br />{" "}
                                  ...
                                  {item.your_price > 0 ? (
                                    ...
                                    <td style={{ fontSize: 13, color: "red" }}>
                                      {" "}
                                    </td>
                                  )}
                                  {!item.weight ? (
                                  ...
                                  <td>
                                    ...
                                    <span
                                      ...
                                      onClick={() =>
                                        this.removeItem(item.item_detail_id)
                                      }
                                    >
                                      <i
                                        class="fa fa-trash"
                              ...
                              <span>
                                ...
                                <strong>
                                  ...
                                  <i
                                    ...
                                    aria-hidden="true"
                                  ></i>{" "}
                                  {this.state.after_login_amount.toLocaleString(
                                    navigator.language,
                                    { minimumFractionDigits: 0 }
                                  )}
                                  .00{" "}
                          ...
                          <tr>
                            ...
                            <td>
                              <span>
                                {" "}
                                <strong>
                                  {this.state.totalWeight.toFixed(2)} kg{" "}
                                </strong>{" "}
                              </span>
                            </td>
                          </tr>
                            ...
                            <tr>
                              ...
                              >
                                ...
                                <input
                                  ...
                                  style={{
                                    backgroundColor:
                                      this.state.isDisabledRFQ == false
                                        ? "#0072bc"
                                        : "#2f2f2f9e",
                                  }}
                                  onClick={this.goToRFQ1}
                                  disabled={this.state.isDisabledRFQ}
                                  value="RFI"
                                  className="login_btn"
                                />{" "}
                              </td>
                              ...
                              <span>
                                ...
                                <strong>
                                  ...
                                  <i
                                    ...
                                    aria-hidden="true"
                                  ></i>{" "}
                                  {this.state.before_login_total_amount}{" "}
                                </strong>{" "}
                              </span>
                            </td>
                          </tr>
        ...
        <Modal isOpen={Successmodal} toggle={this.Successtoggle}>
          <div className="modal__header" style={{ width: "100%", height: 35 }}>
            <button
              ...
              type="button"
              style={{ right: 15 }}
              onClick={this.Successtoggle}
            />
          </div>
          <div
            className="modal__body"
            style={{ paddingTop: 15, paddingBottom: 15, height: 155 }}
          >
            <h4 style={{ fontSize: 18, marginTop: 6, marginBottom: 12 }}>
              {this.state.successFailedMessage}
            </h4>
        ...
        <Modal isOpen={RFQMessagemodal} toggle={this.RFQMessagetoggle}>
          <div className="modal__header" style={{ width: "100%", height: 35 }}>
            <button
              className="fa fa-times modal__close-btn"
              type="button"
              style={{ right: 15 }}
              onClick={this.RFQMessagetoggle}
            />
          </div>
          <div
            className="modal__body"
          ...
          >
            ...
            >
              {this.state.RFQMessage}
            </h4>

            <Button
              style={{
                backgroundColor: "rgb(0, 97, 159)",
                color: "#fff",
                width: 100,
              }}
              className="modal_ok"
              onClick={this.OnRFQMessagetoggle}
            >
              Ok
            </Button>
          </div>
        ...
        <Modal isOpen={showupdateModal} toggle={this.showupdateToggle}>
          <div className="modal__header">
            <button
              className="fa fa-times modal__close-btn"
              type="button"
              style={{ right: 15 }}
              onClick={this.showupdateToggle}
            />
          </div>
          <div
            className="modal__body"
          ...
          >
            <span
              className="fa fa-thumbs-up modal__title-icon"
              style={{
                color: "rgb(0, 97, 159)",
                paddingBottom: 10,
                fontSize: 20,
        ...
        <Modal isOpen={showFailedModal} toggle={this.showfailedToggle}>
          <div className="modal__header">
            <button
              className="fa fa-times modal__close-btn"
              type="button"
              style={{ right: 15 }}
              onClick={this.showfailedToggle}
            />
          </div>
          <div
            className="modal__body"
        ...
        <Modal isOpen={shippingModal} toggle={this.shippingToggle}>
            ...
            <div style={{ width: "100%", marginTop: 10 }}>
              ...
              >
                ...
                {this.state.showError && (
                  <span
                    style={{ fontWeight: "bold", fontSize: 13, color: "red" }}
                  >
                    {" "}
                ...
                {this.state.showupdate && (
                  <span
                    style={{ fontWeight: "bold", fontSize: 13, color: "green" }}
                  >
                    {" "}
        ...
        <Modal isOpen={Failedmodal} toggle={this.Failedtoggle}>
          <div className="modal__header">
            <button
              className="fa fa-times modal__close-btn"
              type="button"
          ...
          >
            <span
              className="fa fa-times-circle-o modal__title-icon"
              style={{
                color: "red",
                paddingBottom: 10,
                fontSize: 25,
        ...
        <Modal isOpen={GoRFImodal} toggle={this.GoRFItoggle}>
          <div className="modal__header" style={{ backgroundColor: "#0072bc" }}>
            <h4
              className="text-modal  modal__title"
              style={{
        ...
        <Modal isOpen={GoCheckoutmodal} toggle={this.GoCheckouttoggle}>
          <div className="modal__header" style={{ backgroundColor: "#0072bc" }}>
            <h4
              className="text-modal  modal__title"
              style={{
        ...
        {this.state.isSession && <SessionModalData />}
      </div>
    );
  }
}
