...
import './styles.scss'
import { Button, Row, Col,Card, CardImg, CardText, CardBody,CardTitle, CardSubtitle, Table} from 'reactstrap';
...
class RFQHistory extends Component {
  ...
  addToCartData(){
    ...
    if(window.location.href == "http://zwz.prtouch.com:8081/rfq-history" || window.location.href == "http://localhost:3000/rfq-history"){
    ...
    }else if (window.location.href == "http://nod.prtouch.com:8081/rfq-history" ) {
      var arr = [];
      for(var i= 0 ; i < postData.length ; i++ ){
        var orderData = {};
   ...
   let posts =this.state.posts.map((postVal, index) => {
     ...
     return (

       {
         ...
         listPrice: <span style={{fontSize: 12}}> <s> <i class="fa fa-inr" style={{fontSize: 12}} aria-hidden="true"></i>  { postVal.list_price.toString().replace(/(\d)(?=(\d\d)+\d$)/g, "$1,") }.00 </s>  <br/>   <span style={{color:'rgb(0, 115, 158)'}}> Savings: { postVal.calculate_discount_percentage.toString()} %</span><br/> <span> <i class="fa fa-inr" style={{fontSize: 12}} aria-hidden="true"></i> {priceVal} </span></span>,
        /* discount: postVal.calculate_discount_percentage.toString(),*/
         totalPrice: postVal.your_price > 0 ?  <span style={{ fontSize: 12}}> <i class="fa fa-inr" style={{fontSize: 12}} aria-hidden="true"></i> { partsval }  </span> : <span style={{color: 'red', fontSize: 14}}> Display on<br/> request </span>,
         /*gst: <span style={{fontSize: 12}}> INR.  { postVal.Total_gst.toString() }  <br/> <span style={{color:'rgb(0, 115, 158)'}}> GST: { postVal.gst_percenatge.toString()} %</span></span>,*/
          gst:  postVal.Total_gst && postVal.gst_percenatge ? <span style={{fontSize: 12}}> <i class="fa fa-inr" style={{fontSize: 12}} aria-hidden="true"></i>  { postVal.Total_gst.toString().replace(/(\d)(?=(\d\d)+\d$)/g, "$1,") }.00  <br/> <span style={{color:'rgb(0, 115, 158)'}}> GST: { postVal.gst_percenatge.toString()} %</span></span> : <span> NA </span>,
         weight: postVal.weight ? <span> {(postVal.weight*postVal.Qty).toFixed(2)} <span> kg </span></span> : <span> NA </span>,
         gst_value: postVal.Total_gst ? postVal.Total_gst : <span style={{ fontSize: 12}}> 0 </span> ,
         status: postVal.status,
         Origin: postVal.Country ? postVal.Country : <span style={{ fontSize: 12}}> NA </span> ,
         Date: <span><span> {postVal.date} </span> <br/> <span> {postVal.time} </span></span>,
         Action:  postVal.status == "Stock" ? <input type="checkbox" checked= {postVal.isChecked} onChange={() => this.checkedSingle(index)} /> : <span style={{fontSize: 12, color: 'red'}}> Out of stock </span>,
 ...
 checkedSingle(index){
  ...
  let selectedItem = this.state.posts.filter(item => item.isChecked == true)
  ...
  /*  var status = localStorage.getItem('status')
    ...
    if (window.location.href==="http://zwz.prtouch.com:8081/rfq-history" ||  window.location.href==="http://localhost:3000/rfq-history" || window.location.href==="http://zwz.prtouch.com:8081/rfq-history#") {
      this.setState({
        isActive: true
      })
      ...
      axios.post('http://apizwz.prtouch.com:8081/api/all_rfq_history/' , 
    ...
    }else if(window.location.href==="http://nod.prtouch.com:8081/rfq-history" ||  window.location.href==="http://nod.prtouch.com:8081/rfq-history#"){
      ...
      axios.post('http://apinod.prtouch.com:8081/api/all_rfq_history/' , 
  ...
  addTocart = rfq_no => {
    const rfqData = _.find(this.state.rfqData, item => item.rfq_no === rfq_no)
    const selectedData = rfqData.data.filter(data => this.state.rfqChecked[rfq_no][data.StkTranNo])
      ...
      <div>
        ...
        <div >
          ...
          {this.state.isActive && 
            ...
            <div className="content-container wrapper" style={{backgroundColor: 'rgb(199, 199, 199)', marginTop: 15, padding: 10, marginBottom: 5}}>

              <div style={{width:'100%'}}>
                <Row>
                  
                  <Col sm={12} style={{marginTop: 15, marginBottom:5}}>
                    <span className="item-action" style={{fontSize: 14}}> <button className="button" type="button" onClick={this.addToCartData}>Add To Cart</button>  </span>
                  </Col>
                    ...
                    <Col sm={12} style={{marginTop: 15, marginBottom:5}}>
                      <Card>
                            <CardBody className="customise_card_body" style={{padding: 18}}>
                              <span style={{fontWeight: 'bold', marginBottom: 20}}> Selected Items ({this.state.selectedItem.length}) </span>
                               ...
                               <div style={{width: '100%', marginTop: 10}}>
                                  {
                                    this.state.posts.map((item, index) => (
                                      item.isChecked == true && (
                                        <span style={{fontWeight: 'bold' , textAlign: 'center', fontSize: 13 , paddingVertical: 12, paddingHorizontal: 15 , paddingLeft: 10, paddingRight: 10, float: 'left' , backgroundColor: '#ccc' , borderRadius: 6, paddingTop: 4, paddingBottom: 4, marginRight: 8, marginBottom: 5}}> {item.ItemName} </span>
