...
class ProductCategory extends Component {
	...
	componentWillMount(){
		...
		this.setState({
			...
			discount_percent: ProductData1.discount_percentage ? ProductData1.discount_percentage[0] : "",
		})
							 ...
							 <Card>
						        <CardBody className="customise_card_body" style={{padding: 18}}>
						          	<CardTitle style={{marginBottom: 20}}>
						          		<div style={{width: '80%' , float: 'left', marginBottom: 12}}> 	
						          			<img src={this.state.image} style={{height:'65px',width:'65px'}} /> 
						          		</div>
						          		<div style={{width: '20%' , float: 'left', textAlign: 'right'}}> 	
						          			<span> <img onClick={this.sendLink} style={{width: 25, height: 25}} src={share} /> </span>
						          		</div>
						          		<h4 style={{fontWeight: 'bold' , width: '100%' , float: 'left'}}> {this.state.itemname}  </h4>
						...
						<Col sm={4} style={{marginTop: 25}}>
							 <Card>
						        <CardBody className="customise_card_body" style={{padding: 18}}>
						          ...
						          <CardSubtitle>
						          		{ this.state.price === 0 &&
						          			<p className="list_price"> List Price: <a style={{fontWeight: 'bold', fontSize: 15, paddingLeft: 4, paddingBottom: 30, borderBottom: 1, color: 'red', fontSize: 13, cursor: 'pointer'}}> Display on request </a> </p>
						          		}

						          		{this.state.price>0 &&
						          			<p className="list_price"> List Price: <a style={{fontWeight: 'bold', fontSize: 15, paddingLeft: 4, paddingBottom: 30, borderBottom: 1}}><i class="fa fa-inr" style={{fontSize: 12}} aria-hidden="true"></i> {this.state.price.toLocaleString(navigator.language, { minimumFractionDigits: 0 })} </a> </p>
						          		}

						          		{
						          			this.state.your_price !== "" &&
						          			<div>
						          			<p className="list_price"> Your Price: <a style={{fontWeight: 'bold', fontSize: 15, paddingLeft: 4, paddingBottom: 30, borderBottom: 1}}> <i class="fa fa-inr" style={{fontSize: 12}} aria-hidden="true"></i> {this.state.your_price.toLocaleString(navigator.language, { minimumFractionDigits: 0 })} ({this.state.discount_percent}% savings) </a> </p>
						          			</div>
						          		}
							 	...
							 	<CardBody className="customise_card_body" style={{padding: 18}}>
						       		...
						       		<div style={{width: '50%', backgroundColor: '#f4f4f4', padding: 18, fontSize: 14, float: 'left'}}>
							          	...
							          	<p><span style={{fontWeight: 'bold', paddingRight: 5}}> Sub Category: </span>{this.state.categoryname} </p>
						       			...
						       			<p><span style={{fontWeight: 'bold', paddingRight: 5}}>  Universal No.: </span> <span> {this.state.universalNo} </span> </p>
						       		...
						       		<div style={{width: '50%', paddingBottom: 55, backgroundColor: '#f4f4f4', padding: 18, fontSize: 14, float: 'left'}}>
						       			...
						       			<p> <span style={{fontWeight: 'bold'}}> Country Association: </span> <span> {this.state.country_origin} </span> </p>

						       			{ this.state.list_price === 0 &&
						          			<p className="list_price" style={{fontWeight: 'bold'}}> List Price: <a style={{fontWeight: 'bold', paddingLeft: 4, paddingBottom: 30, borderBottom: 1, color: 'red', cursor: 'pointer' , fontSize: 12}}> Display on request </a> </p>
						          		}

						          		{this.state.list_price>0 &&
						          			<p className="list_price" style={{fontWeight: 'bold'}}> List Price: <a style={{fontWeight: 'bold', paddingLeft: 4, paddingBottom: 30, borderBottom: 1}}> <i class="fa fa-inr" style={{fontSize: 12}} aria-hidden="true"></i> {this.state.list_price.toLocaleString(navigator.language, { minimumFractionDigits: 0 })}  </a> </p>
						          		}

						
						          		<p><span style={{fontWeight: 'bold', paddingRight: 5}}>  Competitors No.: </span> <span> {this.state.competitorNo} </span> </p>
						...
						<Col sm={12} style={{marginTop: 25}}>
							 <Card>
							 	<CardBody className="customise_card_body" style={{padding: 18}}>
							       		...
							       		<h5 className="product_title_style" style={{width:'85%' , float: 'left'}}> {item.itemname} ({item.brandname}) </h5>
						       		...
						       		<div style={{width: '25%', backgroundColor: '#f4f4f4', padding: 18, fontSize: 14, float: 'left', paddingBottom: 39}}>
...

						       			{
						       				item.Stock === "" ? (
						       					...
						       					<p> <span style={{fontWeight: 'bold', paddingRight: 5}}>Availability:</span> <span> {item.Stock} </span></p>
						       				)
						       			}
						       		...
						       		<div style={{width: '25%', backgroundColor: '#f4f4f4', padding: 18, fontSize: 14, float: 'left', paddingBottom: 75}}>
...

						       			{
						       				item.Weight === "0.000000" ? (
						       					...
						       					<p> <span style={{fontWeight: 'bold', paddingRight: 5}}> Weight:</span> <span> DecimalPrecision.round({item.Weight}) kg </span></p>
						       				)
						       			}

						       			<p> <span style={{fontWeight: 'bold'}}> Universal No.: </span> <span> {item.UniversalNo} </span> </p>

						       			
						       		</div>

						       		<div style={{width: '22%' , float: 'left', marginLeft: 30, border: 1, textAlign: 'center', paddingTop: 10, paddingLeft: 10}} className="add_cart_container">

						       			{ item.price === 0 &&
						          			<p className="list_price" style={{textAlign: 'left'}}> List Price: <a style={{fontWeight: 'bold', paddingLeft: 4, paddingBottom: 30, borderBottom: 1, color: 'red', fontSize: 12, cursor: 'pointer'}}> Display on request </a> </p>
						          		}

						          		{item.price>0 &&
						          			<p className="list_price" style={{textAlign: 'left'}} > List Price: <a style={{fontWeight: 'bold', paddingLeft: 4, paddingBottom: 30, borderBottom: 1}}> <i class="fa fa-inr" style={{fontSize: 12}} aria-hidden="true"></i> {item.price} </a> </p>
						          		}

						          		{this.state.your_price !== "" &&
						          			<p className="list_price" style={{textAlign: 'left'}}> Your Price: <a style={{fontWeight: 'bold', paddingLeft: 4, paddingBottom: 30, borderBottom: 1}}> <i class="fa fa-inr" style={{fontSize: 12}} aria-hidden="true"></i> {this.state.your_price} </a> </p>
...

						          		{
						          			item.your_price &&
						          			<div>
						          			<p className="list_price"> Your Price: <a style={{fontWeight: 'bold', fontSize: 15, paddingLeft: 4, paddingBottom: 30, borderBottom: 1}}> <i class="fa fa-inr" style={{fontSize: 12}} aria-hidden="true"></i> {item.your_price.toLocaleString(navigator.language, { minimumFractionDigits: 0 })} ({item.discount_percentage}% savings) </a> </p>
						          			</div>
						          		}


							       		<p style={{fontWeight: 'bold', textAlign: 'left', fontSize: 15}}> Quantity: 
							          		...
							          		<input type="button"  onClick={() => this.getItemData1(item.itemid, item.itemname , item.price, item)} style={{width: '96%', fontSize: 12, fontWeight: 'bold' , height: 28}} value="Add to cart" className="login_btn" />
							          	</p>
				...
				<Modal
                    ...
                    >
                  	<div className="modal__header">
                    	<button className="fa fa-times modal__close-btn" type="button" style={{right: 15}} onClick={this.shareToggel} />
                  	</div>
                  	<div className="modal__body" style={{paddingTop: 45, paddingBottom: 15}}>
                  		<div style={{flexDirection: 'row', padding: 10}}>
                  			<div style={{width: '75%', float: 'left'}}>
                    			<input type="text" id="myInput" style={{borderWidth: 1, borderColor: 'black'}} value={this.state.url}/>
                    		</div>

                    		<div style={{width: '20%', float: 'left', textAlign: 'left', paddingTop: 5, paddingLeft: 10}}>
                    			{/*<button>Copy Text*/} <i style={{fontSize: 20}} className="fa fa-clipboard" onClick={this.copyText}></i>{/*</button>*/}
                    		</div>
                    	</div>
                  	</div>
