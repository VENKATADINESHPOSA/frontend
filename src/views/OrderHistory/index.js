...
class OrderHistory extends Component {
	...
	componentDidMount(){
		...
		}else if (window.location.href == 'http://store.nodbearings.net/order-history') {
		...
		}else if(window.location.href==="http://store.nodbearings.net/order-history"  || window.location.href==="http://store.nodbearings.net/order-history#"){
			this.setState({
				isActive: true
			})
			axios.get('http://api.store.nodbearings.net/api/my_order/' , {
			 	headers: {
	          		'Authorization' : 'Token ' + localStorage.getItem('auth_key')
	        	} 
			}).then((response) =>  {
 ...
 assemblePosts= () => {
    ...
    let posts =this.state.posts.map((post) => {
    	...
    	if (window.location.href == 'http://store.zwz.co.in/order-history' || window.location.href == 'http://store.zwz.co.in/order-history#' || window.location.href.indexOf("http://localhost:3000/order-history") > -1 ) {
    		var url = 'http://api.store.zwz.co.in/api/print_sales_invoice/'+getInvoiceId+'/'
    	}else if (window.location.href == 'http://store.nodbearings.net/order-history' || window.location.href == 'http://store.nodbearings.net/order-history#') {
    		var url = 'http://api.store.nodbearings.net/api/print_sales_invoice/'+getInvoiceId+'/'
    	}
      ...
      return (

        {
         ...
         /* price: <span style={{fontSize: 13}}> INR.  { post.list_price.toString() } </span>,*/
          price: <span style={{fontSize: 12}}><s> <i class="fa fa-inr" style={{fontSize: 12}} aria-hidden="true"></i>  { post.list_price.toString().replace(/(\d)(?=(\d\d)+\d$)/g, "$1,") }.00 </s>  <br/> <span style={{color:'rgb(0, 115, 158)'}}> Savings: { post.calculate_discount_percentage}%</span><br/> <span> <i class="fa fa-inr" style={{fontSize: 12}} aria-hidden="true"></i> {post.your_price.toString().replace(/(\d)(?=(\d\d)+\d$)/g, "$1,") }.00 </span></span>,
          total: post.your_price > 0 ?  <span style={{ fontSize: 14}}> <i class="fa fa-inr" style={{fontSize: 12}} aria-hidden="true"></i> {(post.your_price * post.quantity).toString().replace(/(\d)(?=(\d\d)+\d$)/g, "$1,") }.00  </span> : <span style={{color: 'red', fontSize: 12}}> Display on request </span>,
          gst: <span style={{fontSize: 12}}> <i class="fa fa-inr" style={{fontSize: 12}} aria-hidden="true"></i>  { post.total_gst1.toString().replace(/(\d)(?=(\d\d)+\d$)/g, "$1,") }.00   <br/> <span style={{color:'rgb(0, 115, 158)'}}> GST: { post.gst_percenatge.toString()} %</span></span>,
          weight:<span> {(post.weight * post.quantity).toFixed(2)} kg</span>,
          origin: post.Country ? post.Country : <span style={{ fontSize: 12}}> NA </span> ,
          date: <span><span> {post.date} </span> <br/> <span> {post.time} </span></span>,
          action: post.status == "Awaiting Internal Approval" ? <a> {post.status} </a> : <a  onClick={() => this.openInvoice(url)} style={{color: 'rgb(0, 115, 158)' }}> {post.status} </a> 
        }
			...
			<div>
				...
				//this.state.rfqData.map((item, index) => (
					<div className="content-container wrapper" style={{backgroundColor: 'rgb(199, 199, 199)', padding: 10, marginBottom: 5}}>
							...
							<div style={{width:'100%'}}>
								<Row>
								<Col sm={12} style={{marginTop: 15, marginBottom:5,display:'none'}}>
										<span className="item-action" style={{fontSize: 14}}> <button className="button" type="button" onClick={this.addToCartData}>Download Invoice</button>  </span>
									</Col>
