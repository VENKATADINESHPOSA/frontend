...
class ReviewRFI extends Component {
	...
	goToRFICheckout(){
		...
		} else if (window.location.href ==="http://nod.prtouch.com:8081/rfq-review") {
		 
		axios.post('http://apinod.prtouch.com:8081/api/customer_master/' , 
	...
	render(){
		...
		return(
				...
				<div className="content-container wrapper" style={{backgroundColor: '#f4f4f4', padding: 10, marginTop: 195}}>
					<Row>					
						...
						<Col sm={10} style={{marginTop: 6}}>
							<h4 style={{paddingLeft: 16, fontWeight:'bold', color: '#00619F'}}>  Review and confirm RFI </h4>
						</Col>
          ...
          <tr className="cart-data">
            ...
            <td>  <input style={{marginTop: 0}} disabled type="text" defaultValue={item.quantity} onKeyPress= {(e)=>this.onUpdateQuantity(e,item.item_detail_id , e.target.value)} className="qty_input" /> </td>
            {item.list_price > 0 ? (
						...
						<Col sm={4} style={{marginTop: 25}}>
							 <Card>
							 	<Col sm={12} style={{paddingLeft: 6, paddingRight: 0, backgroundColor: '#00619F'}} >
						        	<h6 style={{color: '#fff', padding: 6, fontWeight:'bold', textAlign: 'center', paddingTop: 10}}> RFI Summary </h6>
						        </Col> 
						        <CardBody className="customise_card_body" style={{padding: 18}}>
						        	<table style={{width: '100%'}}>
						        		<tbody style={{fontSize: 15}}>
						        			...
						        			<tr>
						        				...
						        				<td colSpan="3" style={{paddingLeft: 0, paddingRight: 0}}> <input type="button" disabled = {this.state.isDisabled} style={{backgroundColor: this.state.isDisabled == false ? '#0072bc' : '#2f2f2f9e'}} onClick={this.goToRFICheckout} value="Proceed" className="login_btn" /> </td>
